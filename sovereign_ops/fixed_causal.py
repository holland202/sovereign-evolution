...
v2: candidate search order is now sorted (was iterating over a Python set,
which has run-to-run non-deterministic order under hash randomization --
this caused find_valid_backdoor_set to return DIFFERENT but equally valid
adjustment sets across separate runs of the same DAG. For an audit tool,
the same input must always produce the same explanation, so this is fixed
to be deterministic even though both answers were mathematically valid.)
"""
